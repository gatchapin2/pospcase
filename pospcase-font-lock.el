...
(defvar pospcase--iterating-p nil
  "Internal variable for deciding if the iterator is on-going.")
...
(defvar pospcase--no-iteration-p nil
  "When the variable is set to non-`nil', the `pospcase--matches'
...
(defmacro pospcase--call-iterator (clause limit &optional allow-atom-p)
  ...
  `(condition-case nil
       (progn
         (unless (or pospcase--no-iteration-p
                     pospcase--matches
  ...
  `(cond
    ...
    ((memq (cdr submatcher) pospcase-defstruct-group)
     `(progn
        ...
        (unless pospcase--fence-start (setq pospcase--no-iteration-p t))
        (condition-case nil
            (progn
              (goto-char (car ,(car submatcher)))
              (backward-up-list)
              (scan-sexps (point) 1))
          (error (setq pospcase--no-iteration-p t)
                      ...
                      '((heading-keyword . (font-lock-keyword-face))
                        ((binds . list/2) .
                         ((pospcase-font-lock-variable-face-form (match-string 2))
                          font-lock-constant-face))))
  (pospcase-font-lock 'lisp-mode
                      '(`(defmethod (setf ,name) ,(pred atom) ,args . ,_)
                        `(defmethod (setf ,name) ,args . ,_)
                        `(defmethod ,name ,(pred atom) ,args . ,_)
                        `(defmethod ,name ,args . ,_)
                        `(defgeneric ,name ,args . ,_)
                        `(cl-defmethod (setf ,name) ,(pred atom) ,args . ,_)
                        `(cl-defmethod (setf ,name) ,args . ,_)
                        `(cl-defmethod ,name ,(pred atom) ,args . ,_)
