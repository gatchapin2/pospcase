...
supported (maybe `pcase' doesn't support it too?)."
  (cl-labels ((walker (node &optional last)
                      (if (consp node)  ; note that in elisp `,foo' is `(\, foo)'
                          (cond
                           ...
                           (t (cons (cl-loop with result
                                             for temp = (reverse node) ; for last cdr cell. What a clever technique!
                                             then (if (memq (cadr temp) '(quote \` \,))
                                                      (cddr temp)
                                                    (cdr temp))
                                             if (memq (cadr temp) '(quote \` \,))
                                             do (setq result (append (walker (list (cadr temp) (car temp)) t) result))
                                             else if (car temp)
                                             do (setq result (append (list (walker (car temp))) result))
                                             while temp
                                             finally return result)
                                    ',_)))
                        (cons node ',_))))
        ...
        (lambda (case)
          (list
           ...
           (cond
            ...
            (t (error "This macro is designed for extracting positional metadata, and not to be used as generic control structure. So complex operations are not supported."))))))
        cases)))
