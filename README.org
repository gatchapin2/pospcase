...
** Submatchers
   ...
   submatchers for each arbitrary length list. So far, seven
...
*** ~parameter~ (~list/3~)
    ...
    #+BEGIN_SRC emacs-lisp
      (destructuring-bind (foo (bar
                                &key (baz 1) (quux 2)))       ; here, the symbol &key change the rule
          '(1 (2 :quux 4 :baz 3))
...
* Writing Your Own SubMatchers
  I’m certain eventually you’ll outgrow and find the syntaxes that
  the build-in ~pospcase-font-lock~ submatchers doesn’t quite cover.

  In that case, it’s time to write your own submatcher using raw
  functions like ~pospcase~, ~pospcase-at~, ~pospcase-read~, or even other
  functions you can find in the wealth of Emacs Lisp ecosystem.

  Since S-expression is infinity flexible, I just can’t write
  universally applicable advice how to write a submatcher.

  I think bisecting and explaining the segments of actual working code
  along with the how the code’s control flow will be like, is the
  best.

  The code of ~pospcase-match-list/1~:
...
*** No support for multiple submatcher
   ...
   implement support for multiple submatchers.
