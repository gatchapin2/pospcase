...
** Add-Form
   ...
   If you read the previous section surely you can guess what each of
   form represents. So add/delete/edit them to make your own
   highlighting rule.
...
* Writing Your Own Anchored-Matchers
  I’m certain eventually you are going to outgrow and find the
  syntaxes that doesn’t quite fit with the build-in ~pospcase-font-lock~
  anchored-matchers.

  In that case, it’s time to write your own anchored-matcher using
  functions like ~pospcase~, ~pospcase-at~, ~pospcase-read~, and even other
  you can find in the wealth of Emacs Lisp ecosystem.

  Since S-expression is infinity flexible, I just can’t write
  universally applicable advice on how to write anchored-matcher.

  I think bisecting and explaining the actual working code, and the
  control flow of invocation, is the best way to go.
