...
* How to Enable Pospcase Font Lock
  ...
  But, by all means I don’t oppose writing your own minor-mode. You
...
* Use case
  ~pospcase~ is written because the author wanted easier way to add code
  highlighting for Lisp code, to embrace Lisp’s extensibility.
...
* Before Writing Your Font Lock Keywords
  Unfortunately current ~pospcase-font-lock~ limitation doesn’t allow
  you to simply write ~pcase~ patterns and let Emacs to DWIM.

  There are some concept you need to know before writing
  ~pospcase-font-lock~ keywords.
